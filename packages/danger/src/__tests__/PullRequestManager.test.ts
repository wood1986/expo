...
describe('parseChangelogSuggestionFromDescription', () => {
  describe('parses title', () => {
    it.each([
      [
        `${message}`,
        {
          [DEFAULT_CHANGELOG_ENTRY_KEY]: { type: DEFAULT_ENTRY_TYPE, message },
        },
      ],
      [
        `[features] ${message}`,
      ...
      [
        `[features][breaking] ${message}`,
        {
          [DEFAULT_CHANGELOG_ENTRY_KEY]: { type: ChangelogEntryType.BREAKING_CHANGES, message },
        },
      ],
    ])('pull request tile: %s', (title: string, expected: ChangelogEntries) => {
      checksParseChangelogSuggestionFromDescription(title, '', expected);
    });
  });

  describe('parses title with package name', () => {
    it.each([
      [
        `[expo-image-picker] ${message}`,
        {
          [DEFAULT_CHANGELOG_ENTRY_KEY]: { type: DEFAULT_ENTRY_TYPE, message },
          'expo-image-picker': { type: DEFAULT_ENTRY_TYPE, message },
        },
      ],
      [
    ...
    ])('%s', (title: string, expected: ChangelogEntries) => {
      checksParseChangelogSuggestionFromDescription(title, '', expected);
    });
  });
  ...
  describe('parses body with multiple entries', () => {
    it.each([
      ...
      [
        ...
        {
          [DEFAULT_CHANGELOG_ENTRY_KEY]: { type: DEFAULT_ENTRY_TYPE, message },
          'expo-image-picker': { type: DEFAULT_ENTRY_TYPE, message: 'Fix expo-image-picker' },
          'expo-permissions': {
            type: ChangelogEntryType.BREAKING_CHANGES,
            message: 'Fix expo-permissions',
          },
